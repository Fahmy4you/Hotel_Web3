 ...
 *
 * /kategori/update/{id_kategori}:
 *   put:
 *     tags: [Kategori]
 *     summary: Update kategori - Access Role [Admin, Pemilik Hotel]
 *     description: Update an existing kategori.
 *     security:
 *       - Bearer: []
 *     parameters:
 *       - in: path
 ...
 *         schema:
 *           type: integer
 *         description: Numeric ID of the kategori to Update
 *     requestBody:
 *       required: true
 *       content:
 *         application/json:
 ...
 *                 message:
 *                   type: string
 *                   example: Kategori updated successfully
 *                 kategoriKamar:
 *                   type: object
 *                   properties:
 *                     id:
...
export async function PUT(
  ...
  const kategoriIDToUpdate = Number(idKategori.id_kategori);
  const existingKategori = await prisma.kategoriKamar.findFirst({
    where: {
      id: kategoriIDToUpdate,
    },
  });
  ...
  try {
    const result = await prisma.kategoriKamar.update({
      where: {
        id: kategoriIDToUpdate,
      },
      data: {
    ...
    console.error(error);
    return NextResponse.json(
      { error: "Failed to updated kategori" },
      { status: 500 }
    );
  } finally {
